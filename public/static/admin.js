...
/*! 动态插件库路径 */
window.tapiRoot = window.appRoot;
...
require.config({
    ...
    paths: {
        ...
        'upload': [tapiRoot + 'tool/upload?.js'],
        ...
        'xm-select': ['plugs/select/xm-select'],
    },
    shim: {
...
$(function () {
    ...
    $.msg = new function (that) {
        ...
        this.tips = function (msg, time, callback) {
            var index = layer.msg(msg, {icon: 0, time: (time || 3) * 1000, shade: this.shade, end: callback, shadeClose: true});
            return this.idx.push(index), index;
        };
        /*! 显示正在加载中的提示 */
        ...
        this.auto = function (ret, time) {
            ...
            if (parseInt(ret.code) === 0 && time === 'false') {
                return url ? (location.href = url) : $.form.reload();
            }
            return (parseInt(ret.code) === 0) ? this.success(msg, time, function () {
        ...
        this.load = function (url, data, method, callback, loading, tips, time, headers) {
            ...
            $.ajax({
                data: data || {}, type: method || 'GET', url: $.menu.parseUri(url), beforeSend: function (xhr, i) {
                    ...
                    return typeof ret === 'object' ? $.msg.auto(ret, time || ret.wait || 2) : that.show(ret);
                }, complete: function (ret) {
                    //处理 新表单令牌
                    if(ret.responseJSON && typeof ret.responseJSON.new_token !== "undefined" && $('form input[name="_token"]')[0]){
                        $('form input[name="_token"]').val(ret.responseJSON.new_token);
                    }
                    $.msg.close(index);
                }
            });
    ...
    $body.on('click', '[data-url]', function () {
        let data = {}, time = this.dataset.time, action = this.dataset.url, table = this.dataset.table;
        let loading = this.dataset.loading, method = this.dataset.method || 'post';
        let rule = this.dataset.value || (function (elem, rule, ids) {
            let checked_item = window.table.checkStatus(table).data;
            for (let index in checked_item) {
                ids.push(checked_item[index].id)
            }
            return ids.length > 0 ? rule.replace('{key}', ids.join(',')) : '';
        })(this, this.dataset.rule || '', []) || '';
        if (rule.length < 1) return $.msg.tips('请选择需要更改的数据！');
        ...
        console.log(data);
        this.dataset.confirm ? $.msg.confirm(this.dataset.confirm, function () {
            $.form.load(action, data, method, callback, load, tips, time);
        }) : $.form.load(action, data, method, callback, load, tips, time);
    });
    ...
    $body.on('blur', '[data-action-blur]', function () {
        ...
        that.callback = function (ret) {
            return $this.css('border', (ret && ret.code == 0) ? '1px solid #e6e6e6' : '1px solid red'), false;
        };
        data['_token_'] = this.dataset.token || this.dataset.csrf || '--';
        this.dataset.confirm ? $.msg.confirm(this.dataset.confirm, function () {
    ...
    $body.on('click', '[data-icon]', function () {
        var location = tapiRoot + 'tool/icon', field = this.dataset.icon || this.dataset.field || 'icon';
